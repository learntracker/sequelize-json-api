  ...
  function fixlinks(req, model, instance){
    ...
    if(Object.keys(model.associations).length > 0){
      ...
      for(association in model.associations){
        ...
        var tableName = model.tableName.toLowerCase();

        if(assoc.associationType == 'BelongsTo'){
          if(identifier in values){
            values.links[identifier] = {
              href: util.format("http://%s%s/%s/%s", req.host, options.endpoint, targetName, values[identifier]),
        ...
        if(assoc.associationType == 'HasMany'){
          if(targetName in values){
            values.links[targetName] = {
              href: util.format("http://%s%s/%s/%s/%s", req.host, options.endpoint, tableName, values.id, targetName),
              ids: [],
              type: targetName
            };
